...
(select 	
	...
	case @e_empresa when '8459' then ts.ts_nombre else "'"+isnull(ts_referencia,'')  end	 IDENTIFICACION,  --RECMPS-1467
...
select 	
	...
	case @e_empresa when '8459' then ts.ts_nombre else "'"+isnull(ts_referencia,'')  end	 IDENTIFICACION, --KB RECMPS-1467
	ts_campo_alt_uno								REFERENCIA,  --ref 2
	"BOL" 											REFERENCIA_AUX,
	"USD"   										MONEDA_PAGO,
...
begin
	...
	from (select 	
		...
		case @e_empresa when '8459' then hs_nombre else "'"+isnull(hs_referencia,'')  end	 IDENTIFICACION, --RECMPS-1467
