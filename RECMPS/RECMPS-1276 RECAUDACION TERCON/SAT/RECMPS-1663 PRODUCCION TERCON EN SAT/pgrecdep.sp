...
begin
  ...
  begin
	...
	--INI REF4
    if  @e_debito <> 0 
		--and (@e_canal = 'VEN' or @e_canal = 'IBK' or @e_canal = 'SAT' or @e_canal = 'WAP') --KBastida RECM-1453 Ref SAT
    begin
		if @e_canal = 'ATM'
			select @v_desc_canal = 'VEINTI4EFECTIVO'
		if @e_canal = 'DBA'
			select @v_desc_canal = 'AUTOMATICO'
		if @e_canal = 'IBK'
			select @v_desc_canal = '24Online'  
		if @e_canal = 'IVR'
			select @v_desc_canal = 'VEINTI4FONO'
		if @e_canal = 'KSK'
			select @v_desc_canal = 'PUNTOVEINTI4'
		if @e_canal = 'VEN'
			select @v_desc_canal = 'VENTANILLA'
		if @e_canal = 'WAP'
			select @v_desc_canal = 'WAP'
		if @e_canal = 'CNB'
			select @v_desc_canal = 'CNB'
		if @e_canal = 'SAT'  --KBastida RECM-1453 Ref SAT
			select @v_desc_canal = 'SAT'
