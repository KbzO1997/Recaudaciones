...
if not(object_id('pa_cash_cprocesar_tarifaBE') is NULL) 
	drop proc dbo.pa_cash_cprocesar_tarifaBE
go
...
if exists(select 1 from sysobjects where name = 'pa_cash_cprocesar_tarifaBE')
	select 'El pa_cash_cprocesar_tarifaBE se compilo Exitosamente'
else 
	select 'El pa_cash_cprocesar_tarifaBE NO se compilo!!!'
go

grant exec on dbo.pa_cash_cprocesar_tarifaBE to apl_osb_hpact
go
