...
/*          Stored Procedure    : pa_cash_cprocesar_tarifaBP.sp                     */
...
/*    8  23/Feb/2022 Kevin Bastida   RECMPS-1370    CONSULTA Y PAGO RECAUDACION TERCON */
/***********************************************************************************/

if exists(select 1 from sysobjects where name = 'pa_cash_cprocesar_tarifaBP')

    drop proc dbo.pa_cash_cprocesar_tarifaBP

go

create  proc dbo.pa_cash_cprocesar_tarifaBP(
...
select 
	...
	if(@e_codigo_empresa_proveedora = 1111	OR @e_codigo_empresa_proveedora = 1112 OR @e_codigo_empresa_proveedora = 1113 OR @e_codigo_empresa_proveedora = 1114 OR @e_codigo_empresa_proveedora = 1929) --kbastida
		begin
...
go 
if exists(select 1 from sysobjects where name = 'pa_cash_cprocesar_tarifaBP')
  print 'El sp pa_cash_cprocesar_tarifaBP se compilo Exitosamente'
else 
  print 'El sp pa_cash_cprocesar_tarifaBP NO se compilo!!!'
go
grant execute on pa_cash_cprocesar_tarifaBP to apl_osb_hpdes
go
