...
/*11 28-Abr-2015  Maria Jose Silva    RECA-CC-SGC00020190  Cobro de comisi�n IESS*/
/*12 15-Sep-2015  Pablo Holguin V.    CTE-CC-SGC00022250 Modificaciones */
/*14 15-Sep-2015  Juan Camilo V�lez.  SAT-SD-SGC00020798 Modificaciones */
...
/*18 25-May-2015  Mar�a Jos� Silva  ETAPA-AP-SGC00018437-SGC00018451 Comisi�n de Etapa*/
...
/*33 16-Feb-2017  Daniel Pereira     RECA-CC-SGC00027934  Cobro Comisi�n ND Ventanilla */
...
/* 36 11/Abr/2017 Mar�a Jo. Silva G. RECA-CC-SGC00027715 Servicios Claro	     */
/* 37 14-Mar-2017 Pablo Holguin      CTE-CC-SGC00028182 Modificacion de servicios    */
/* 38 21/Abr/2017 Miriam Lindao   SAT-CC-SGC00028083 Reportar Comisi�n efectivo o cheque Colegios*/
...
/* 42 18/May/2017 Mar�a Jos� Silva RECA-CC-SGC00028461  Cobro de comision de TVCABLE   	*/
/* 43 18/May/2017 Mar�a Jos� Silva RECA-CC-SGC00027998  Traslado comision VEN DIRECTV  	*/
/* 44 04/JUL/2017 Vanessa Miranda INRECL-AP-SGC00027264-SGC00027269  Reportar en A06   	*/
/*    las transacciones de WAP que no tengan comisi�n, solo contrar las transacciones  	*/
/*    porque las que tiene comisi�n est�n parametrizadas                               	*/
...
/*     Para los servicios b�sicos considerar que si est� parametrizado el cobro de     	*/
/*     comisi�n no deber�a salir estas transacciones en el ccmana06, solo considerar las*/
...
/*     este caso el A06 manual debe extraer la informaci�n                             	*/
...
select @valor2 =0 --Ref24 Cambio cta b�sica VMT
--< Temporales
...
where hm_tipo_tran in (4272, 4264, 4262,120060) --Ref24 se agrega transacci�n cta b�sica VMT
...
where hm_tipo_tran in (4272, 4264, 4262,120060) --Ref24 se agrega transaccion 120060 cta b�sica VMT
  and hm_causa in ('57', '472', '0')
  and hm_correccion = 'N'
  and not exists(select 1 from ##correccion  where hs_ssn_corr = mov.hm_secuencial)
...
where hm_tipo_tran in (4272, 4264, 4262,120060) --Ref24 se agrega trx cta b�sica VMT
  and hm_causa in ('57', '472', '0')
  and hm_correccion = 'N'
  and not exists(select 1 from ##correccion  where hs_ssn_corr = mov.hm_secuencial)
  group by hm_valor
...
select
	...
	@i_descripcion		= 'Servicio de retiro de dinero en cajeros automaticos propios de clientes de una entidad de red local',
	@i_tarifa		= 0.50
...
select
	...
	@i_descripcion		= 'Servicio de retiro de dinero a no clientes del Banco, de instituciones financieras del exterior',
	@i_tarifa		= 2
...
where hs_tsfecha >= @i_fecha_ini
 ...
 --- Cobro de comisi�n a la empresa utpl
...
where hs_tsfecha >= @i_fecha_ini
 ...
 --- Cobro de comisi�n a la empresa por canal ven
  ...
  where hs_tsfecha >= @i_fecha_ini
    ...
    and hs_causa in ('7256', '1089')                                                  -- JPM Ref.31: Megadatos: incluir 1089
...
if @valor2 > 0 --requiere que al final del c�lculo de la tarifa promedio ponderada se realice la deflactaci�n considerando el par�metro de IVA
  begin
    -- Se obtiene el % del IVA
    select @w_porcIva = pa_float + 1
      from cobis..cl_parametro
...
where hs_tsfecha >= @i_fecha_ini
 ...
 --- Cobro de comisi�n a la empresa por canal internet
...
select
	...
	@i_descripcion		= 'Recaudaciones de pagos a terceros, a excepci�n de recaudaciones de tributos pagados con tarjetas',	---< ref:37 >--Nueva descripcion por cambio de codigo de servicio
...
--ref29 inicio
---SE CUENTAN TODAS LAS TRANSACCIONES Y SE SUMAN SOLO LAS QUE TENGAN COMISI�N
...
-->ref46 inicio-->PARA CONTAR LAS TRANSACCIONES QUE NO TENGAN COMISION, AQUI NO IMPORTA EL VALOR, PORQUE EL VALOR SOLO SE CONSIDERA CUANDO HAY COMISION
  ...
  where hs_tsfecha >= @i_fecha_ini
    ...
    and hs_secuencial not in (select hs_ssn_corr from ##correccion)
    GROUP BY hs_secuencial having count (1)=1-->CUANDO UNA TRANSACCI�N NO TIENE COMISI�N SOLO SE GENERA UN REGISTRO.
-->ref46 fin
...
---CONTAR TODAS LAS TRANSACCIONES, TENGAN O NO COMISI�N Y SUMAR MONETARIAMENTE SOLO LAS TRANSACCIONES QUE TENGAN COMISI�N
...
-- Si el monto de la transferencia est� entre $0.01 y $1,000.00 	--> TA00970000
-- Si el monto de la transferencia est� entre $1,000.01 y $5,000.00 	--> TA00980000
-- Si el monto de la transferencia est� entre $5,000.01 y $10,000.00	--> TA00990000
...
select
	...
	@i_descripcion		= 'Recaudaciones de pagos a terceros, a excepci�n de recaudaciones de tributos pagados con tarjetas' ,--'Aplica cuando el cliente accede a diversos servicios de instituciones publicas o privadas', ---< ref:37 >--Moficacion descripcion
...
where hm_fecha >= @i_fecha_ini
  ...
  and hm_moneda = 1
  and hm_tipo_tran in (3050)
  and hm_causa in ('297','440', '241', '110', '289', '242', '447')--ref46 sacan las causas(149,466,229)se comenta causa que tiene comisiones parametrizadas en Autom�tico
...
select
	...
	@i_descripcion		= 'Recaudaciones de pagos a terceros, a excepci�n de recaudaciones de tributos pagados con tarjetas',  ---< ref:37 >--Nueva descripcion
...
select
    ...
    @i_descripcion  = 'Recaudaciones de pagos a terceros, a excepci�n de recaudaciones de tributos pagados con tarjetas' , --< ref:37 >--Modificacion descripcion
...
where hs_tsfecha >= @i_fecha_ini
  ...
  and hs_usuario = 'cnb001'
  and hs_correccion = 'N'
  and hs_valor > 0 -- s�lo lee los pagos en efectivo
  and hs_secuencial not in (select hs_ssn_corr from ##correccion)
...
select
	...
	@i_descripcion		= 'Emisi�n de plastico de tarjeta prepago no recargable con banda lectora (incluye la carga)',  --< ref:37 >--Modificacion descripcion
...
select
    ...
    @i_descripcion      = 'Suspensi�n transitoria del pago de cheque(s) (por evento para uno o varios cheques)',	--< Ref:37 >--Modificaicon Descripcion codigo
...
from
    cob_ahorros_acum..ah_his_mov_a06
where hm_tipo_tran in (4272, 4264, 4262, 4372,120060)--Ref24 se agrega trx cta b�sica VMT
  and hm_causa in ('945', '946', '947')
  and hm_correccion = 'S'
...
select
    ...
    @i_descripcion  = 'Recaudaciones de pagos a terceros, a excepci�n de recaudaciones de tributos pagados con tarjetas' ,--'Pago de tramite y citaciones', --< ref:37 >--Modificacion descripcion
...
--------------------------------------------------------------------------ref44 inicio
/*POR EL CANAL WAP SE HAN PARAMETRIZADO TRANSACCIONES QUE TIENE COMISI�N PERO NO SE EST� OBTENIENDO LAS TRANSACCIONES
 QUE NO TIENEN COMISI�N LAS CUALES HAY QUE CONTAR, POR TAL MOTIVO SE VA A INGRESAR UN REGISTRO PARA CONTEMPLAR LAS
 TRANSACCIONES SIN COMISI�N.
...
select
	...
	@i_descripcion		= '(Declaraci�n sin efecto de cheque(s))(por evento para uno o varios cheques)',
	@i_tarifa		=  2.68 --Ref54 cambio tarifa  3
...
if @@sqlstatus = 1
	...
	--Ref50 maxima tarifa
	select  @i_tarifa= max(isnull(cd_tarifa,0))	from cob_super..cc_detalle_a06
	where cd_fecha_corte = @i_fecha_fin
	and cd_canal=@i_canal
	and cd_tipo_servicio=@i_servicio
	and cd_prop_canal = @v_prop_canal
