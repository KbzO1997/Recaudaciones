...
/*    Procesamiento: 	  OLTP	                                        */
/*    Ult.ControlTarea:   J-RECMPS-2042                          		*/
/*    Ult.Referencia:     REF 43                          			    */
/*    Producto:           Credito                                       */
...
/*	41 23/11/2022  Luis Cepeda F	RECM-657 GENERACION DE COMPROBANTE DE RETENCION MOVISTAR/TUENTI*/
/*	42 23/12/2022  Luis Cepeda F	RECM-660 PAGO A LA EMPRESA DESDE EL TSERVI PARA CLARO*/
/*	43 12/01/2023  Kevin Bastidas	RECMPS-2042 PAGO LC CON FORMA PAGO INTERBANCARIO				*/
/*************************************************************************/
...
begin
    ...
    begin tran
      ...
      values (@s_ssn,1, @t_trn, @w_ofi, @s_user, @s_rol, @s_term,
         ...
         if @@error <> 0
         begin
               if @@trancount > 0  /*<REF 42>*/
                    rollback tran  /*<REF 42>*/
         /* Error en insercion de transaccion de servicio */
             exec cobis..sp_cerror
             @t_debug = @t_debug,
             @t_file  = @t_file,
  ...
  begin
      exec @w_return = cob_cuentas..sp_factura_recaudacion
       ...
       -- <-- JPM Ref.28
		...
		--<REF 43, KBastida Validacion Para Pago InterBancario Ini		
		if @w_pn_forma_pago = 'PGINTB'
		begin
			set @w_pn_diferencia = 0
			set @w_ts_monto = @w_pn_valor
		end
		--REF 43, Validacion Para Pago InterBancario REF 43>
