...
if exists (select 1 from dbo.sysobjects where name = 'pa_pag_lc_dep_temporal')
	drop procedure dbo.pa_pag_lc_dep_temporal
	if exists (select 1 from sysobjects where name = 'pa_pag_lc_dep_temporal')
		PRINT '<<< DROP PROCEDURE pa_pag_lc_dep_temporal -- ERROR -- >>>'
	else
		PRINT '== DROP PROCEDURE pa_pag_lc_dep_temporal *OK* =='

go
/************************************************************************/
/*  Archivo:            pglcdeptmp.sp                                     */
/*  Motor de Base: 		SYBASE                     					    */
/*  Servidor: 			HP-ACT                                       	*/
/*  Procesamiento: 		OLTP	                                       	*/
/*  Ult.ControlTarea:   J-RECMPS-2027                        		*/
/*  Stored procedure:   pa_pag_lc_dep_temporal                          */
/*  Aplicacion: 		Cyberbank	                                	*/
/*  Base de datos:      cob_pagos                                       */
/*  Producto:           Recaudaciones                              		*/
/*  Disenado por:       Kevin Alexander Bastidas Z.                     */
/*  Fecha de escritura: 16-Dic-2012                                     */
/*-----------------------------------------------------------------------*/
/*                            IMPORTANTE                                 */
...
/* 0  16/Dic/2022  Kevin Bastidas  RECMPS-2027   Emisión Inicial         */
/*************************************************************************/


create proc dbo.pa_pag_lc_dep_temporal (
...
if exists(select 1 from sysobjects where name='pa_pag_lc_dep_temporal')
	PRINT '== CREATE PROCEDURE pa_pag_lc_dep_temporal *OK* =='
else
	PRINT '<<< CREATE PROCEDURE pa_pag_lc_dep_temporal -- ERROR -- >>>'
go