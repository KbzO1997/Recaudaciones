...
/* Stored procedure:   pa_pg_tdeptemp_lc                           		*/
/* Procesamiento:      OLTP                                             */
/* Ult.ControlTarea:   J-RECMPS-2027                                    */
/* Ult.Referencia:     REF 2                                            */
/* Disenado por:       Kevin Alexander Bastidas                       	*/
/* Fecha de escritura: 16/Dic/2022                                      */
/************************************************************************/
/*			                IMPORTANTE		       		                */
...
/************************************************************************/
/* PROPOSITO:                                                           */
/*Este programa realiza la recaudación de pago automático Deposito Temporal*/
/************************************************************************/
/*  MODIFICACIONES                                                      */
/************************************************************************/
/*  REF   FECHA             AUTOR                           TAREA       */
/*    1   16/Dic/2022       Kevin Bastidas  			RECMPS-2027     */
/*        Emision inicial                                               */
/*    2   28/Dic/2022       Kevin Bastidas  			RECMPS-2027 ACREDITACION AUTOMATICA DEPOSITOS TEMPORALES */
/************************************************************************/
use cob_pagos
go
if exists(select 1 from sysobjects where name = 'pa_pg_tdeptemp_lc' and type = 'P')
	begin
		drop procedure dbo.pa_pg_tdeptemp_lc
		if exists(select 1 from sysobjects where name = 'pa_pg_tdeptemp_lc' and type = 'P')
			PRINT '<<< DROP PROCEDURE pa_pg_tdeptemp_lc --ERROR-- >>>'
			else
			PRINT '== DROP PROCEDURE pa_pg_tdeptemp_lc *OK* =='
	end   
go
create procedure dbo.pa_pg_tdeptemp_lc
...
select @v_flag = 0
select @v_sp_name = 'pa_pag_lc_dep_temporal', @v_file = 'pglcdeptmp.sp'
...
/*<REF 2, KBastida Validacion de empresas por catalogo REF 2>*/
if exists(select 1
       from cobis..cl_catalogo b 
			inner join cobis..cl_tabla a 
			on b.tabla = a.codigo
			where a.tabla = 'cl_empresas_pago_automatico'
			and b.codigo = convert(varchar(7), @e_empresa) 
			and estado != 'V')
begin
	print "ERROR EMPRESA DE DESTINO NO HABILITADA"
	return 1
end 
/*<REF 2, Validacion de empresas por catalogo REF 2>*/
...
begin
	...
	select @v_pe_fpago   = pe_fpago,
		...
		@v_acedita_grp 	 = (case 
							when isnull(@v_cod_grupo, '0') = '0' then (select isnull(pe_acedita_grp, 'N') 
																										  from cob_pagos..pg_person_empresa 
																										  where pe_empresa = @e_empresa )  
																	  else (select isnull(pe_acedita_grp, 'N') 
																											  from cob_pagos..pg_person_empresa 
																											  where pe_empresa = convert(int,@v_cod_grupo ) )
							end )  , 
	...
	begin
		...
		begin
			...
			where ts_tipo_transaccion in (3925,3926,3928,43547,3194,3203)
			and ts_correccion       = 'N'
		...
		begin
			exec cobis..sp_cerror
				...
				@t_file          = @v_file ,
				@t_from          = @v_sp_name ,
				@i_msg           = 'ESTA PARAMETRIZADO COMO ACREDITACIÓN GRUPO',
				@i_num           = 111111
			return 1
		end
	end
...
BEGIN TRAN
	...
	begin
		...
		begin
			select  @v_fecha_pago = dateadd(dd, 1, @v_fecha_pago)
			if not exists (select 1 --df_fecha
							from  cobis..cl_dias_feriados
	...
	insert into cob_pagos..pg_control_ncrecaudacion(
...
go
if exists(select 1 from sysobjects where name='pa_pg_tdeptemp_lc' and type = 'P')
	PRINT '== CREATE PROCEDURE pa_pg_tdeptemp_lc *OK* =='
	else
	PRINT '<<< CREATE PROCEDURE pa_pg_tdeptemp_lc -- ERROR -- >>>'
go