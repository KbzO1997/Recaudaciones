...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="RespuestaOK">
		...
		<ns17:montoMinimo>
			<xsl:text disable-output-escaping="no">0.01</xsl:text>
			<visible>
		...
		<ns17:formaPagoRecibos>
			<xsl:text disable-output-escaping="no">UNICO_CRONOLOGICO</xsl:text>
		</ns17:formaPagoRecibos>
		<ns17:nombreCliente>
			<xsl:value-of select="/mcnel:MensajeSalidaConsultar/informacionDeposito/ns4:nombreCliente"/>
		</ns17:nombreCliente>
		<xsl:if test="/mcnel:MensajeSalidaConsultar/operacion ='SAT'">
			<ns17:datosAdicionales>
				<ns17:datoAdicional>
					<codigo>
					  <xsl:text disable-output-escaping="no">fecCont</xsl:text>
					</codigo>		
					<valor>
				...
				<ns17:datoAdicional>
					<codigo>
					  <xsl:text disable-output-escaping="no">tipDoc</xsl:text>
					</codigo>		
					<valor>
					  <xsl:value-of select="/mcnel:MensajeSalidaConsultar/informacionDeposito/ns4:tipDoc"/>
					</valor>
					<visible>
					  <xsl:text disable-output-escaping="no">false</xsl:text>
					</visible>
				...
				<ns17:datoAdicional>
					<codigo>
					  <xsl:text disable-output-escaping="no">numDoc</xsl:text>
					</codigo>		
					<valor>
					  <xsl:value-of select="/mcnel:MensajeSalidaConsultar/informacionDeposito/ns4:numDoc"/>
					</valor>
					<visible>
					  <xsl:text disable-output-escaping="no">false</xsl:text>
					</visible>
		...
		<xsl:if test="/mcnel:MensajeSalidaConsultar/operacion!='SAT'">	 
			<ns17:recibos>
				<xsl:for-each select="/mcnel:MensajeSalidaConsultar/informacionDeposito">
					<ns17:recibo>

						<comprobante>
							<xsl:value-of select="ns4:numeroDocumento"/>
						</comprobante>

						<fecha>
							<xsl:value-of select="ns4:fecCont"/>
						</fecha>

						<secuencia>
							<xsl:value-of select="ns4:tipDoc"/>
						</secuencia>

						<identificador>
							<xsl:value-of select="ns4:numDoc"/>
						</identificador>

						<totalAPagar>
							<xsl:value-of select="ns4:deuda"/>
