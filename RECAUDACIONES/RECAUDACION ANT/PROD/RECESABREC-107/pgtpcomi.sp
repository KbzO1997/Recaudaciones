...
if @e_comision_tot > 0
	begin
		...
		if @e_tipo_cta = 'EF' and exists ( select 1  /*<REF 2, KBastida incluir Efectivo comision REF 2>*/
		...
		if @e_tipo_cta = 'CH' and exists ( select 1 
											 from cobis..cl_catalogo b
									   inner join cobis..cl_tabla a 
											   on b.tabla = a.codigo
											where a.tabla = 'sv_trx_cont_iva_rms'
	   ...
	   --<REF 3, KBastida incluir Cuenta Corriente y Ahorro comision
	   if @e_tipo_cta = 'AHO' and exists ( select 1  
											 from cobis..cl_catalogo b
									   inner join cobis..cl_tabla a 
											   on b.tabla = a.codigo
											where a.tabla = 'sv_trx_cont_iva_rms'
		...
		if @e_tipo_cta = 'CTE' and exists ( select 1  
											 from cobis..cl_catalogo b
									   inner join cobis..cl_tabla a 
											   on b.tabla = a.codigo
											where a.tabla = 'sv_trx_cont_iva_rms'
					 ...
					 begin   
						...
						begin 		 				 
							  ...
							  exec @v_return = cob_cuentas..sp_cont_impuesto
											   ...
											   @i_cau         = @v_servicio, --@v_causal_com, REF 3 KBastida
											   @i_cta         = @e_num_operacion,  
											   @i_mon         = @e_mon,   --Moneda
											   @i_oficina_cta = @e_ofi,
											   @i_ubi         = @e_ubi,
