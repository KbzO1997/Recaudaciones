...
where hs_tsfecha >= @i_fecha_ini
  ...
  and hs_moneda = 1
  and hs_tipo_transaccion in (3366, 3372)
  and hs_causa in ('14', '15', '50') /*<REF 69, Se agrega la causa 50 para Claro Srv Fijos REF 69>*/
  and hs_usuario not in ('ope998', 'SAT', 'srolese', 'IVR', 'kiosko','WAP', 'SROLESE') -- ref055 jg
  and hs_correccion = 'S'
   and hs_terminal <> 'ATMB'  --ref 45
...
from cob_cuentas_his..cc_his_movimiento a
		where hm_fecha >= @i_fecha_ini and hm_fecha <= @i_fecha_fin
		and hm_tipo_tran = 3188 and hm_causa = '985'
		and hm_usuario not in ('ope998', 'srolese', 'IVR', 'SAT', 'kiosko','cnb001','SROLESE','WAP')
		and hm_correccion = 'S'
		and hm_terminal <> 'ATMB'  
...
select 
	count(1),
	sum(isnull(hm_valor, 0)),
	(select of_provincia 
		from cobis..cl_oficina 
		where of_oficina = a.hm_oficina),
...
from cob_cuentas_his..cc_his_movimiento a
		...
		and hm_terminal <> 'ATMB'  
		and hm_correccion = 'N'
		and hm_secuencial not in(select hs_ssn_corr from ##correccion) 
		group by hm_oficina, hm_valor


insert into ##temp_resporder
select 
	sum(isnull(cantidad,0)),sum(isnull(valor,0)),provincia,canton, (sum(isnull(valor,0))/@w_porcIva)/sum(isnull(cantidad,0))
	from ##3datos
	group by provincia,canton, valor
...
select 
	...
	cd_tipo_cliente=null,cd_numero_trans = cantidad, 
	cd_tarifa=tarifa,
	cd_ingreso_total=round(sum(isnull(valor, 0))/@w_porcIva,2),
	cd_tasa_ponderada=0,
	cd_cta_contable='540702',
	cd_prop_canal='P',
	cd_cod_provincia=provincia,
	cd_cod_canton=canton,
	cd_ingreso_neto=round(sum(isnull(valor, 0))/@w_porcIva,2), 
	cd_base_imponible=round(sum(isnull(valor, 0))/@w_porcIva,2),
	cd_iva_pagado= round(sum(isnull(valor, 0)) - sum(isnull(valor, 0))/@w_porcIva,2),cd_monto_base=0,cd_cargo_porcentaje=0
	from ##temp_resporder
	group by provincia,canton,tarifa
...
insert into ##correccion
select hm_sec_correccion
from cob_ahorros_his..ah_his_movimiento a
	where hm_fecha >= @i_fecha_ini and hm_fecha <= @i_fecha_fin
	and hm_tipo_tran = 4188 and hm_causa = '982'
	and hm_usuario not in ('ope998', 'srolese', 'IVR', 'SAT', 'kiosko','cnb001','SROLESE','WAP')
	and hm_correccion = 'S'
	and hm_terminal <> 'ATMB'  
...
from cob_cuentas_his..cc_his_movimiento a
	where hm_fecha >= @i_fecha_ini and hm_fecha <= @i_fecha_fin
	and hm_tipo_tran = 3188 and hm_causa = '644'
	and hm_usuario not in ('ope998', 'srolese', 'IVR', 'SAT', 'kiosko','cnb001','SROLESE','WAP')
	and hm_terminal <> 'ATMB'  
	and hm_correccion = 'S'
...
from cob_ahorros_his..ah_his_movimiento a
where hm_fecha >= @i_fecha_ini and hm_fecha <= @i_fecha_fin
	and hm_tipo_tran = 4188 and hm_causa = '197'
	and hm_usuario not in ('ope998', 'srolese', 'IVR', 'SAT', 'kiosko','cnb001','SROLESE','WAP')
	and hm_terminal <> 'ATMB'  
	and hm_correccion = 'S'
...
insert into ##correccion
select hm_sec_correccion
from cob_virtuales_his..vi_his_movimiento a
	where hm_fecha >= @i_fecha_ini and hm_fecha <= @i_fecha_fin
		and hm_tipo_tran in (120061,120072)
		and ltrim(rtrim(hm_causa)) = '51'
		and hm_usuario not in ('ope998', 'srolese', 'IVR', 'SAT', 'kiosko','cnb001','SROLESE','WAP')
		and hm_terminal <> 'ATMB'
		and hm_correccion = 'S'
