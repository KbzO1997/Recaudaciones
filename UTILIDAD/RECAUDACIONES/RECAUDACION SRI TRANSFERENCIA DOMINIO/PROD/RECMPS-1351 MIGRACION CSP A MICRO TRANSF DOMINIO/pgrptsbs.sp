...
/*21  30/Ene/2023  Luis Cepeda	       RECMPS-1950 BACKOFFICE BANCO PAGO MUNICIPIO CON TC EN SAT*/
/*22  21/Mar/2023  Luis Cepeda	       RECMPS-2114 GENERACION BACKOFFICE VENTANILLA*/
/*23  28/Jun/2023  Dario Espinosa      RECM-739 CONCEPTO DE ESTRUCTURA PAGO A TERCERO*/
/*24  06/Jun/2023  Kevin Bastidas      RECMPS-1351 MIGRACION CSP A MICRO TRANSFERENCIA DE DOMINIO*/
/************************************************************************************/
use cob_pagos
go
...
create procedure dbo.sp_reporte_mensual_sbs (
@i_fecha  datetime
)
as
...
insert into cob_cuentas..cc_inf_super
    ...
    select @w_fecha_corte ,'CTA',@w_sri_codigo,@w_bco_nombre,@w_sri_nombre, @w_sri_ruc, 
           @w_sri_tipemp, ts_secuencial, convert(varchar(10),ts_tsfecha,103),
            ...
            0, /*<Ref 22 Se recorre una columna REF 22>*/
            case when ts_monto > 0 then 1 else 0 end, /*<Ref 22 is_ModPago_tarjcredito_cte REF 22>*/
            --Ref018:msilvag Fin
            0,0,/*<REF 22 0, REF 22>*/
			...
			--Ref018:msilvag Fin
			0,/*<REF 22 '0,00', REF 22>*/
            case when (ts_monto > 0) and (ts_monto > ts_saldo) and (ts_monto > ts_valor) and (ts_monto > ts_ocasional) then @v_tarifa_sri 
                 		 else '0,00' end, /*<REF 22 Se recorre una columna REF 22>*/
          ...
          select @w_fecha_corte, 'CTA',@w_sri_codigo,@w_bco_nombre,
            ...
            where ts_tipo_transaccion = t.tp_tipo_trx
             ...
             --and tc_tipo_tran in (3295,3336)
              and tc_causa_org = '0' /*<REF 24 Migracions CPS A MICRO TRNSF Dominio isnull(ts_causa,'0') REF 24>*/ 
              and tc_estado       = 'V'
              and tc_contabiliza = 'ADU'
      ...
      insert into cob_cuentas..cc_inf_super
          ...
          select @w_fecha_corte, 'CTA',@w_sri_codigo,@w_bco_nombre,
               ...
               case when ts_usuario = 'ope998' and ts_monto > 0 then 1
               		when ts_usuario in ('srolese','SROLESE','sa',/*<REF 22 'WAP','wap', REF 22>*/'consola') then 1 /*<Ref018:msilvag Inicio  Ref018>*/
			   		when ts_usuario like '0000%' then 1
			   		when ts_usuario like '%CAJ00%' then 1
			   		when ts_monto > 0 then 1 
			   		--when substring(ts_tipo_chequera,1,3) = 'IBK' and convert(money,ts_autoriz_aut) > 0  then 1
          ...
          select @w_fecha_corte, 'CTA',@w_sri_codigo,@w_bco_nombre,
                ...
                --is_ModPago_tarjcredito_cte
                0,case when ts_contratado > 0 /*<REF 21 and ts_usuario = 'ope998' REF 21>*/ then 1 else 0 end, 
                --Ref018:msilvag Fin
                0,0,/*<REF 21 0, REF 21>*/
                --Ref018:msilvag Inicio  is_ModPago_debito_pago_clte
                case  when ts_usuario in ('srolese','SROLESE', 'sa','ope998','WAP','wap','consola') then 0 --Ref018:msilvag Inicio
			   		when ts_usuario like '0000%' then 1
					when ts_usuario like '%CAJ00%' then 1
					when ts_monto > 0 then 1 
					--when substring(ts_tipo_chequera,1,3) = 'IBK' and convert(money,ts_autoriz_aut) > 0  then 1
...
--Ref006:msilvag Inicio
    ...
    begin
        ...
        select @w_fecha_corte, 'CTA',@w_sri_codigo,@w_bco_nombre,
                ...
                (select of_provincia  from cobis..cl_oficina where of_oficina = isnull(p.ts_oficina,0)),
                (select substring(of_canton,3,2) from cobis..cl_oficina where of_oficina = isnull(p.ts_oficina,0)),
                @w_cta_contable, @v_concepto + ' ' + @w_concepto_reca, /*<REF 23 concepto de varias empresariales REF 23>*/ 
                tp_cod_archivo,cp_causa,ts_usuario 
        from db_temporal..cc_tran_servicio_tmp p, cob_pagos..pg_person_empresa x,
              db_temporal..cc_trans_pagos_terc t,
              db_temporal..cc_causas_pagos_terc c
