...
public class WebServiceConfig {
	...
	private synchronized void initPool() {
		try {
			if (poolClienteLatinia == null) {
				poolClienteLatinia = new GenericObjectPool<>(
						new WSClientPoolFactory<>(NotificacionesPortType.class, latiniaURL));
	...
	public OTPManager clientebayted(String proceso) {
		...
		try {
			...
			logger.tiempo(proceso, nameBayted, CoreUtils.dateToString(startTime),
					CoreUtils.getTimeIntervalInt(startTime), null, baytedConnTimeout - 1000);

			return clienteBayted;
		} catch (Exception e) {
			logger.error(proceso, nameBayted, CoreUtils.dateToString(startTime),
					CoreUtils.getTimeIntervalInt(startTime), null, e.toString());
			return null;
		}
