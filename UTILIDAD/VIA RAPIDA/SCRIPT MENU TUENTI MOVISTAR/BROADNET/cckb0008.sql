...
DECLARE
  ...
  LV_COBIS_EMPRESA			    VARCHAR2(5)  := '19575';
  LV_EMPRESA 				        VARCHAR2(50) := 'Tuenti Compra Paquete';
...
BEGIN
  ...
  select MAX(FSE_ID)+1 into LN_CODIGO_FLUJOENRIQ from DB_OTC.OTC_D_FLUJO_SERV_ENRIQ;
    ...
    values (LN_CODIGO_CONVENIO,LV_COBIS_EMPRESA,'1','Combo Tuenti');
...
/
	...
	select EMP_CODIGO from DB_OTC.OTC_M_EMPRESA  where EMP_CODIGO = '19575';
	select CNV_CODIGO from DB_OTC.OTC_M_CONVENIO  where CNV_CODIGO = '19575';
	select SRV_NOMBRE from DB_OTC.OTC_D_SERVICIO where SRV_NOMBRE = '19575';
	
  /*	
  select * from DB_OTC.OTC_M_EMPRESA where EMP_ID = 467;
  select * from DB_OTC.OTC_M_GRUPO_SERVICIO where GRP_ID = 734;
  select * from  DB_OTC.OTC_M_CONVENIO where CNV_ID = 361;
  select * from DB_OTC.OTC_D_SERVICIO where SRV_ID = 942;
  select * from  DB_OTC.OTC_D_SERVICIO_CANAL where SC_ID = 165;
  select * from DB_OTC.OTC_M_TIPO_IDENTIFICADOR where TID_ID = 454;
  select * from DB_OTC.OTC_M_PUNTO_FINAL where END_ID in (215,216);
  select * from DB_OTC.OTC_M_FLUJO where FLU_ID in (1090, 1091);
  select * from DB_OTC.OTC_D_FLUJO_SERV_ENRIQ where FSE_ID in (505,506);


  delete  from DB_OTC.OTC_D_FLUJO_SERV_ENRIQ where FSE_ID in (505,506);
  delete from DB_OTC.OTC_M_FLUJO where FLU_ID in (1090, 1091);
  delete from DB_OTC.OTC_M_PUNTO_FINAL where END_ID in (215,216);
  delete from DB_OTC.OTC_M_TIPO_IDENTIFICADOR where TID_ID = 454;
  delete from  DB_OTC.OTC_D_SERVICIO_CANAL where SC_ID = 165;
  delete from DB_OTC.OTC_D_SERVICIO where SRV_ID = 942;
  delete from  DB_OTC.OTC_M_CONVENIO where CNV_ID = 361;
  delete from DB_OTC.OTC_M_GRUPO_SERVICIO where GRP_ID = 734;
  delete from DB_OTC.OTC_M_EMPRESA where EMP_ID = 467;
  */
