...
/* Fecha de escritura: 06/10/2022                                         */
/* Descripcion:        Script para agregar a la empresa al  menu OTC      */
/**************************************************************************/
/*                            MODIFICACIONES                              */
/*   FECHA     TAREA             AUTOR                  RAZON             */
/*05/31/2022  RECESABREC-35 		Kevin Bastidas    Emision inicial         */
/**************************************************************************/
...
DECLARE
  ...
  LN_GRUPO_SERVICIO			    NUMBER := 24;  --TELEFONIA_CELULAR
  LV_COBIS_EMPRESA			    VARCHAR2(5)  := '19574';
  LV_EMPRESA 				        VARCHAR2(50) := 'Movistar Compra Paquete';
  LV_RUC_EMPRESA			      VARCHAR2(15) := '0999999999001';
  ...
  LV_EMPRESA_XSL			      VARCHAR2(50) := 'RMS_BROADNET';
BEGIN
  ...
  select MAX(FSE_ID)+1 into LN_CODIGO_FLUJOENRIQ from DB_OTC.OTC_D_FLUJO_SERV_ENRIQ;
    ...
    values (LN_CODIGO_CONVENIO,LV_COBIS_EMPRESA,'1','Combo Movistar');
    ...
    values (LN_CODIGO_IDENTIFICADOR,'CELULAR',null,'0000000000','1','^09([0-9]){8}$$',null,LN_CODIGO_CONVENIO,'Ingrese número de celular','No.celular','0');
...
/
	...
	select EMP_CODIGO from DB_OTC.OTC_M_EMPRESA  where EMP_CODIGO = '19574';
	select CNV_CODIGO from DB_OTC.OTC_M_CONVENIO  where CNV_CODIGO = '19574';
	select SRV_NOMBRE from DB_OTC.OTC_D_SERVICIO where SRV_NOMBRE = '19574';
	
  /*
 select * from DB_OTC.OTC_M_EMPRESA where EMP_ID = 466;
  select * from DB_OTC.OTC_M_GRUPO_SERVICIO where GRP_ID = 733;
  select * from  DB_OTC.OTC_M_CONVENIO where CNV_ID = 360;
  select * from DB_OTC.OTC_D_SERVICIO where SRV_ID = 941;  
  select * from  DB_OTC.OTC_D_SERVICIO_CANAL where SC_ID = 164;  
  select * from DB_OTC.OTC_M_TIPO_IDENTIFICADOR where TID_ID = 453;
  select * from DB_OTC.OTC_M_PUNTO_FINAL where END_ID in (213,214);  
  select * from DB_OTC.OTC_M_FLUJO where FLU_ID in (1088, 1089);
  select * from DB_OTC.OTC_D_FLUJO_SERV_ENRIQ where FSE_ID in (503,504);


  delete  from DB_OTC.OTC_D_FLUJO_SERV_ENRIQ where FSE_ID in (503,504);
  delete from DB_OTC.OTC_M_FLUJO where FLU_ID in (1088, 1089);
  delete from DB_OTC.OTC_M_PUNTO_FINAL where END_ID in (213,214);
  delete from DB_OTC.OTC_M_TIPO_IDENTIFICADOR where TID_ID = 453;
  delete from  DB_OTC.OTC_D_SERVICIO_CANAL where SC_ID = 164;
  delete from DB_OTC.OTC_D_SERVICIO where SRV_ID = 941;
  delete from  DB_OTC.OTC_M_CONVENIO where CNV_ID = 360;
  delete from DB_OTC.OTC_M_GRUPO_SERVICIO where GRP_ID = 733;
  delete from DB_OTC.OTC_M_EMPRESA where EMP_ID = 466;
  */