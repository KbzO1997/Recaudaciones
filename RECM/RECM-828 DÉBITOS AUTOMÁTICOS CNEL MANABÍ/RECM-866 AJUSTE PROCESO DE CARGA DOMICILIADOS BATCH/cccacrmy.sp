...
/* 3  28/Feb/2023  Kevin Bastidas          RECM-866 Ajuste en proceso de carga domiciliados batch*/
/*----------------------------------------------------------------------------------*/
...
begin
  ...
  begin tran /*<REF 3, Add tran REF 3>*/ 
   ...
   WHERE re_secuencial IN (
         ...
         WHERE re_estado IN ('I', 'T', 'W')
            AND ((re_fecha_carga != @i_fecha AND re_empresa = @i_empresa))
                OR
                (re_fecha_carga = @i_fecha AND re_empresa = @i_empresa AND re_secuencia_carga != 1))
        AND re_empresa != 1081
 ...
 /*<REF 3, Add tran commit tran REF 3>*/ 
